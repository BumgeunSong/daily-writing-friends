...
Deno.test('buildNotificationMessage', async (t) => {
  ...
  await t.step('긴 콘텐츠는 35자로 잘리고 말줄임표 추가', () => {
    ...
    assertEquals(result, `테스트님이 '${longContent.slice(0, 35)}...' 글에 댓글을 달았어요.`);
  });

  await t.step('35자 이하 콘텐츠는 그대로 표시', () => {
    const shortContent = '짧은 제목';
    const result = buildNotificationMessage('comment_on_post', '테스트', shortContent);
    assertEquals(result, "테스트님이 '짧은 제목' 글에 댓글을 달았어요.");
  });
