...
- **Local seed service_role key is auto-minted**: recent Supabase CLI signs JWTs with ES256 only, so the legacy HS256 `service_role` key from `supabase status` is rejected by the auth admin API (`bad_jwt: signing method HS256 is invalid`). The seed scripts (`seed-e2e-users`, `seed-e2e-domain`, `seed-perf-fixture`) and `tests/helpers/seed-cleanup` now mint their own ES256 key from the running auth container's `GOTRUE_JWT_KEYS` (`scripts/lib/local-service-role.ts`) — no manual key handling. Set `SUPABASE_SERVICE_ROLE_KEY` to override (e.g. in CI).
