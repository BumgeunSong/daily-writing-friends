...
export const adminQueryKeys = {
  ...
  waitingUsers: (boardId: string) =>
    ["admin", "board", boardId, "waiting-users"] as const,
  users: ["admin", "users"] as const,
  userSearch: (q: string) => ["admin", "users", "search", q] as const,
  usersByIds: (ids: string[]) =>
    ["admin", "users", "by-ids", ...ids.slice().sort()] as const,
  previousCohortPosts: (userId: string, cohort: number) =>
    ["admin", "users", userId, "previous-cohort-posts", cohort] as const,
  posts: (boardId: string, range: PostsRange) =>
    ["admin", "posts", boardId, range] as const,
...
export async function getBoard(boardId: string): Promise<GetBoardResponse["board"]> {
  const res = await adminGet<GetBoardResponse>(
    `/api/admin/boards/${encodeURIComponent(boardId)}`
  );
  return res.board;
}
...
export async function getBoardUsers(
  boardId: string
): Promise<GetBoardUsersResponse["users"]> {
...
export async function getWaitingUsers(
  boardId: string
): Promise<GetWaitingUsersResponse["users"]> {
...
export async function searchUsers(
  query: string
): Promise<SearchUsersResponse["users"]> {
...
export async function getUsersByIds(
  ids: string[]
): Promise<GetUsersByIdsResponse["users"]> {
...
export function approveUser(
  ...
  return adminPost<ApproveUserResponse, ApproveUserRequest>(
...
export function rejectUser(
  ...
  return adminPost<RejectUserResponse, RejectUserRequest>(
...
): Promise<CreateBoardResponse["board"]> {
  const res = await adminPost<CreateBoardResponse, CreateBoardRequest>(
    "/api/admin/boards",
    body
  );
  return res.board;
