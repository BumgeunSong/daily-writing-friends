...
const normalizeSanitizedHtml = (html: string): string => {
  ...
  // Safe: html is always DOMPurify.sanitize()'d before reaching here
  div.innerHTML = html;
...
const createUlFromOl = (ol: Element): HTMLUListElement => {
  ...
  // Safe: ol.innerHTML comes from DOMPurify-sanitized content
...
const createTempElement = (html: string): HTMLDivElement => {
  ...
  // Safe: callers pass DOMPurify.sanitize()'d content
