...
/**
 * Flatten the per-page results returned by useInfiniteQuery into one ordered
 * list. Hook-internal: the public hook surface returns the flat array; callers
 * should not need this helper directly.
 */
export const flattenNotificationPages = (
  pages: NotificationPage[] | undefined,
): Notification[] => pages?.flatMap((page) => page.notifications) ?? [];
