...
/**
 * 권한 오류 화면이 막는 것은 무한 왕복이다. 홈 탭의 목적지 `/boards`는 두 저장소를
 * 참고하는데, 볼 수 없게 된 게시판을 가리키는 기억이 하나라도 남으면 확인을 눌러도
 * 곧장 이 화면으로 되돌아온다. 사용자는 빠져나갈 방법이 없다.
 *
 * 그래서 여기서 확인하는 것은 대화상자 모양이 아니라 두 저장소가 모두 비워지는지다.
 */
...
describe('참여하지 않은 기수라 읽기 권한이 없을 때', () => {
  ...
  it('확인을 누르면 그 게시판을 가리키던 기억이 모두 사라진다', async () => {
