...
export async function simulateHistoricalStreak(
  ...
  completeDayBuckets.forEach(bucket => {
    console.log(`- ${bucket.kstDateString}: ${bucket.events.length} posts, working: ${bucket.isWorkingDay}`);
  });
    ...
    if (dayBucket.kstDateString >= '2025-07-25' && dayBucket.kstDateString <= '2025-08-05') {
      ...
      console.log(`DEBUG ${dayBucket.kstDateString}:`, {
        ...
        before: { 
          ...
          missedDate: beforeStatus.type === 'eligible' ? beforeStatus.missedDate?.toDate().toLocaleDateString('sv-SE', { timeZone: 'Asia/Seoul' }) : undefined
        },
        after: { 
          status: afterStatus.type, 
          streak: dayResult.newState.currentStreak,
          originalStreak: dayResult.newState.originalStreak,
          missedDate: afterStatus.type === 'eligible' ? afterStatus.missedDate?.toDate().toLocaleDateString('sv-SE', { timeZone: 'Asia/Seoul' }) : undefined
        },
 ...
 */
function handleOnStreakToContinue(
  ...
  return {
    ...
    originalStreak: state.originalStreak + 1,
...
function handleOnStreakToEligible(
  ...
  const missedDateTimestamp = Timestamp.fromDate(new Date(`${dayBucket.kstDateString}T00:00:00+09:00`));
 ...
 */
function handleMissedToNewStreak(
  state: SimulationState,
  dayBucket: DayBucket
): SimulationState {
  return {
...
function createRecoveryFromDayResult(
  ...
  if (previousState.status.type === RecoveryStatusType.ELIGIBLE && previousState.status.missedDate) {
    missedDateString = previousState.status.missedDate.toDate().toLocaleDateString('sv-SE', { timeZone: 'Asia/Seoul' });
  } else {
...
function generateCompleteDayTimeline(dayBuckets: DayBucket[]): DayBucket[] {
  ...
  const endDate = new Date(sortedBuckets[sortedBuckets.length - 1].kstDateString + 'T00:00:00+09:00');
