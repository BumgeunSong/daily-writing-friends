...
export async function runBackfillProcess(
  params: import('./types').BackfillParams,
): Promise<import('./types').BackfillResponse> {
  const { extractPostingEvents, groupEventsByDay, getEarliestPostingDate } = await import(
    './eventExtraction'
  );
  ...
  try {
    ...
    // Horizon: today (KST) by default. Use asOfDate (KST date) as horizon to extend timeline to today
    const horizonKst = params.asOfDate.toLocaleDateString('sv-SE', { timeZone: 'Asia/Seoul' });
    const simulationResult = await simulateHistoricalStreak(dayBuckets, initialState, horizonKst);
...
// Helper functions
function createEmptyBackfillResponse(
  params: import('./types').BackfillParams,
): import('./types').BackfillResponse {
  return {
    userId: params.userId,
