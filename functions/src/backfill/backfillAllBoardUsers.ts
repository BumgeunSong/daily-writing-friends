...
export const backfillAllBoardUsersHttp = onRequest(
  ...
  async (request, response) => {
    ...
    try {
      ...
      console.log(`Backfilling board: ${boardId}, dryRun: ${dryRun}, maxUsers: ${maxUsers}`);
      ...
      for (const user of usersToProcess) {
        try {
          ...
          const testResult = await runBackfillProcess({
            userId: user.uid,
            asOfDate: new Date(), // Current time
            dryRun: true
          });
      ...
      console.log(`Users with data: ${usersWithData.length}, without data: ${usersWithoutData.length}`);
      ...
      for (const user of usersWithData) {
        try {
          ...
          console.log(`✅ Backfill successful for ${user.uid}: ${result.finalState.currentStreak} streak`);
