  ...
  private async callGeminiWithRetry(prompt: string): Promise<LLMAnalysisResult> {
    ...
    for (let attempt = 1; attempt <= PROCESSING_LIMITS.retryAttempts; attempt++) {
      try {
        ...
        return this.validateAndParseResponse(text);
        
      } catch (error) {
        lastError = error as Error;
        console.error(`Gemini API call failed (attempt ${attempt}/${PROCESSING_LIMITS.retryAttempts}):`, error);
    ...
    throw new Error(`Gemini API failed after ${PROCESSING_LIMITS.retryAttempts} attempts. Last error: ${lastError?.message}`);
  }
  ...
  private validateAndParseResponse(responseText: string): LLMAnalysisResult {
    try {
      ...
      const validTones: PostTone[] = [
        'thoughtful', 'warm', 'emotional', 'humorous', 'serious',
        'informal', 'formal', 'optimistic', 'calm', 'guiding', 'friendly'
      ];
      ...
      const validMoods: PostMood[] = [
        'happy_uplifting', 'sad_gloomy', 'tense_exciting', 'romantic_loving',
        'mysterious_curious', 'funny_lighthearted', 'peaceful_calm'
      ];
