...
- [ ] T.1 **Vitest**: `apps/web/src/shared/api/__tests__/postgrestFilters.test.ts` — `escapeForOrFilter` is a pure `(string) → string`. Cases: empty, plain ASCII, plain Korean, each of `\` `%` `_` `,` `(` `)` `*` `"` `:` `.` `\n` `\t` in isolation, mixed Korean + wildcards, leading/trailing whitespace (preserved at this layer — trimming happens upstream), and the injection-attempt fixture string `,author_id.eq.00000000-0000-0000-0000-000000000000` whose escaped form MUST NOT produce a parseable second filter clause.
- [ ] T.2 **Vitest**: `apps/web/src/user/components/__tests__/deriveSearchState.test.ts` — `deriveSearchState(query, { isFetching, isError, data? })` is a pure `(string, plain-shape) → union`. Cover: all five terminal states; the evaluation-order property (idle gate wins over previous `data` — the `keepPreviousData` regression guard); the `isError` branch winning over `isFetching`; the `keepPreviousData` mid-typing case (`isFetching: true` with prior `data` returns `'results'`, not `'loading'`).
