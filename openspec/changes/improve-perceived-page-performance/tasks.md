...
- [x] 1.4 Heavy-dep decisions for PR2 `manualChunks` — design's "likely" list (`@sentry/react/replay`, `recharts`, `react-markdown`) did not match reality. Replaced with: keep `react-vendor`; drop stale `firebase/firestore`; add `sentry-vendor` and `supabase-vendor`. `heic2any` (1.3 MB!) stays statically imported by `post/utils/ImageUtils.ts` and moves out of `index` naturally once post routes go lazy. Sentry Replay (313 KB) is **removed entirely in PR2** (see task 7 scope change), so no manualChunks entry is needed for it.
