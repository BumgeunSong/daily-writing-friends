...
- **~~Important SDK v8 detail:~~ Correction (2026-05-16, found during implementation):** This decision originally claimed that adding Replay via `Sentry.addIntegration()` after `Sentry.init` would orphan the top-level `replaysSessionSampleRate` / `replaysOnErrorSampleRate` rates. **That claim is wrong for SDK 8.55.** `@sentry-internal/replay@8.55.2/.../esm/index.js:9707` defines `loadReplayOptionsFromClient` which reads those rates from `client.getOptions()` inside the integration's `_setup(client)` step — and `_setup` runs whenever the integration is added (including via `addIntegration`). So the rates are NOT orphaned. `Sentry.replayIntegration({ sessionSampleRate, errorSampleRate })` would also work because the integration's options merge takes precedence, but it is not required. Recorded for accuracy; moot now that Replay is removed entirely.
