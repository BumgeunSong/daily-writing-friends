...
| `/settings/add-password` UI | 별도 페이지 (모달 X) |
...
<Button
  ...
  onClick={() => navigate('/settings/add-password')}
>
...
오른쪽 끝의 `"설정 안 됨"` 레이블 — 현재 상태를 명시. 사용자가 "왜 이 항목이 있지?"에 즉시 답을 주는 텍스트.
...
`"설정됨"` 레이블으로 현재 상태 확인. `KeyRound` (lucide-react)를 사용해 자물쇠/보안 맥락 전달.

### 비밀번호 추가 플로우 (Settings에서)

Settings 행 클릭 → `/settings/add-password` 페이지 (별도 라우트, 모달 아님):

```
┌─────────────────────────────┐
│ 헤더                         │
│ "비밀번호 추가"               │  text-2xl font-semibold
│ "이메일과 비밀번호로도 로그인할  │  text-sm text-muted-foreground
│  수 있어요."                 │
│                             │
│ 새 비밀번호    JoinFormField  │  + 요구사항 힌트 (signup과 동일)
│ 비밀번호 확인  JoinFormField  │
│                             │
│ [비밀번호 설정]               │  variant="cta", w-full
└─────────────────────────────┘
```
...
성공 시: `toast.success('비밀번호가 설정되었습니다. 이제 이메일로도 로그인할 수 있어요.')` → Settings로 돌아감. Settings의 비밀번호 행은 이제 "설정됨" 상태로 갱신.

`variant="cta"` 사용: 사용자에게 가치 있는 새 능력을 활성화하는 전환 행동.
